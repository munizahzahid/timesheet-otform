<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('All Approved OT Forms') }}</h2>
    </x-slot>
            ...
            <div class="p-6">
                <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('records.ot-forms') }}" class="px-3 py-1.5 rounded-md text-sm font-medium bg-indigo-600 text-white">{{ __('OT Form') }}</a>
                    </div>
                    <form method="GET" action="{{ route('records.ot-forms') }}" class="flex flex-wrap items-center gap-2">
                        ...
                        @if($month || $year)
                            <a href="{{ route('records.ot-forms') }}" class="px-4 py-1.5 bg-gray-200 text-gray-700 text-sm rounded-md hover:bg-gray-300 text-center">{{ __('Clear') }}</a>
                        @endif
                    </form>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $ot->form_type === 'executive' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                            {{ $ot->form_type === 'executive' ? __('Executive') : __('Non-Executive') }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <div class="flex items-center gap-3">
                                            <a href="{{ route('records.ot-forms.show', $ot) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('View') }}</a>
                                            <a href="{{ route('ot-forms.export-excel', $ot) }}" class="text-green-600 hover:text-green-800">{{ __('Excel') }}</a>
                                            <a href="{{ route('ot-forms.export-pdf', $ot) }}" class="text-red-600 hover:text-red-800">{{ __('PDF') }}</a>
                                        </div>
                                    </td>
                                </tr>
                            @empty
