...
<aside style="position:fixed;top:0;left:0;height:100vh;height:100dvh;width:16rem;overflow-x:hidden;background-color:#1e3a8a;color:#d1d5db;"
       class="z-30 text-gray-300 flex flex-col transform transition-all duration-300 ease-in-out lg:translate-x-0"
       :style="{ width: collapsed ? '5rem' : '16rem' }"
       :class="[sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0', collapsed ? 'sidebar-collapsed' : '']">

    {{-- Logo / Brand --}}
    <div class="brand-block flex items-center justify-between px-6 py-5 border-b border-blue-800 bg-blue-900/50 backdrop-blur">
        <a href="{{ route('dashboard') }}" class="overflow-hidden">
    ...
    <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        ...
        <a href="{{ route('dashboard') }}"
           x-bind:title="collapsed ? 'Dashboard' : null"
        ...
        <a href="{{ route('timesheets.index') }}"
           x-bind:title="collapsed ? 'HR' : null"
           class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group
                  {{ !$viewingOtherUser && (request()->routeIs('timesheets.*') || request()->routeIs('ot-forms.*') || request()->routeIs('training-attendance.*')) ? 'bg-blue-700 text-white shadow-lg shadow-blue-700/40 border-l-4 border-blue-600' : 'text-gray-300 hover:bg-blue-800 hover:text-white' }}">
            <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ !$viewingOtherUser && (request()->routeIs('timesheets.*') || request()->routeIs('ot-forms.*') || request()->routeIs('training-attendance.*')) ? 'text-white' : 'text-gray-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>
            <span class="flex-1 nav-label">HR</span>
            @if($hrNewStatusCount > 0)
                <span class="badge-count inline-flex items-center justify-center flex-shrink-0 text-xs font-bold rounded-full bg-red-500 text-white" style="width: 20px; height: 20px;">
                    {{ $hrNewStatusCount }}
                </span>
                <span class="badge-dot w-2 h-2 rounded-full bg-red-500 flex-shrink-0"></span>
            @endif
        </a>
        ...
        <a href="{{ request()->has('user_id') ? route('history.index', ['user_id' => request('user_id')]) : route('history.index') }}"
           x-bind:title="collapsed ? 'History' : null"
           class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group
        ...
        @endphp
        @if($showApprovals)
            <div class="pt-4 pb-2 section-label">
                <p class="px-4 pb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Approvals</p>
            </div>

            @if($user->role === 'admin' || $user->role === 'hr')
                <a href="{{ route('approvals.pending-tracker.index') }}"
                   x-bind:title="collapsed ? 'Pending Tracker' : null"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group
                          {{ !$viewingOtherUser && request()->routeIs('approvals.pending-tracker.*') ? 'bg-blue-700 text-white shadow-lg shadow-blue-700/40 border-l-4 border-blue-600' : 'text-gray-300 hover:bg-blue-800 hover:text-white' }}">
                    <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ !$viewingOtherUser && request()->routeIs('approvals.pending-tracker.*') ? 'text-white' : 'text-gray-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
                    </svg>
                    <span class="flex-1 nav-label">Pending Tracker</span>
                </a>
            @endif

            @if($user->role === 'admin' || $user->role === 'hr' || $isOtApprover)
                <a href="{{ route('approvals.ot-forms.index') }}"
                   x-bind:title="collapsed ? 'OT Approvals' : null"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group
                    ...
                    <span class="flex-1 nav-label">OT Approvals</span>
            ...
            @if($user->role === 'admin' || $isTimesheetApprover)
                <a href="{{ route('approvals.timesheets.index') }}"
                   x-bind:title="collapsed ? 'Timesheet Approvals' : null"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group
                    ...
                    <span class="flex-1 nav-label">Timesheet Approvals</span>
        ...
        @if(Auth::user()->isAdmin())
            <div>
                <button @click="if (collapsed) { collapsed = false; adminOpen = true; } else { adminOpen = !adminOpen; }"
                        x-bind:title="collapsed ? 'Admin' : null"
                    ...
                    <svg x-show="!collapsed" class="w-4 h-4 transition-transform duration-200" :class="adminOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="adminOpen"
                    ...
                    <a href="{{ route('admin.users.index') }}"
                       x-bind:title="collapsed ? 'Users' : null"
                    ...
                    <a href="{{ route('admin.project-codes.index') }}"
                       x-bind:title="collapsed ? 'Project Codes' : null"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all duration-200
                        ...
                        <span class="nav-label">Project Codes</span>
                    </a>
                    <a href="{{ route('admin.holidays.index') }}"
                       x-bind:title="collapsed ? 'Holidays' : null"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all duration-200
                    ...
                    <a href="{{ route('admin.desknet-sync.index') }}"
                       x-bind:title="collapsed ? 'Desknet Sync' : null"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all duration-200
                        ...
                        <span class="nav-label">Desknet Sync</span>
                    </a>
                    <a href="{{ route('admin.audit.index') }}"
                       x-bind:title="collapsed ? 'Audit Logs' : null"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all duration-200
        ...
        @if(Auth::user()->isAdmin())
            <a href="{{ route('admin.settings.index') }}"
               x-bind:title="collapsed ? 'System Settings' : null"
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group
                      {{ !$viewingOtherUser && request()->routeIs('admin.settings.*') ? 'bg-blue-700 text-white shadow-lg shadow-blue-700/40 border-l-4 border-blue-600' : 'text-gray-300 hover:bg-blue-800 hover:text-white' }}">
                <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ !$viewingOtherUser && request()->routeIs('admin.settings.*') ? 'text-white' : 'text-gray-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
    ...
    {{-- User profile card --}}
    <div class="profile-card flex-shrink-0 px-4 pt-4 pb-4 border-t border-blue-800">
        <div class="flex items-center gap-3">
            <a href="{{ route('profile.edit') }}" title="My Profile" class="flex-shrink-0">
            ...
            <div class="profile-detail overflow-hidden">
                <p class="text-sm font-semibold text-white truncate">{{ Auth::user()->name }}</p>
                <p class="text-xs text-gray-400 truncate">{{ Auth::user()->designation ?? Auth::user()->email }}</p>
            </div>
            <div class="ml-auto flex items-center gap-1">
                <button @click="collapsed = !collapsed"
                        class="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-blue-800 focus:outline-none transition"
                ...
                <form method="POST" action="{{ route('logout') }}" class="profile-detail">
                    @csrf
                    <button type="submit" title="Logout" class="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-blue-800 transition">
