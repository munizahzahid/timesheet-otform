...
<div class="overflow-x-auto">
    <table class="w-full border-collapse text-xs">
                    ...
                    <td class="border border-gray-200 px-1 py-1.5 text-center text-xs">
                        <span id="ot1-{{ $entry->id }}">
                            @if($entry->ot_normal_day_hours > 0)
                                {{ number_format(floor($entry->ot_normal_day_hours * 4) / 4, 2) }}
                            @else
                                <span class="text-gray-400">-</span>
                            @endif
                        </span>
                        <input type="hidden" id="ot1-input-{{ $entry->id }}" name="entries[{{ $entry->id }}][ot_normal_day_hours]" value="{{ number_format(floor(($entry->ot_normal_day_hours ?? 0) * 4) / 4, 2) }}">
                    </td>
                    <td class="border border-gray-200 px-1 py-1.5 text-center text-xs">
                        <span id="ot2-{{ $entry->id }}">
                            @if($entry->ot_rest_day_hours > 0)
                                {{ number_format(floor($entry->ot_rest_day_hours * 4) / 4, 2) }}
                            @else
                                <span class="text-gray-400">-</span>
                            @endif
                        </span>
                        <input type="hidden" id="ot2-input-{{ $entry->id }}" name="entries[{{ $entry->id }}][ot_rest_day_hours]" value="{{ number_format(floor(($entry->ot_rest_day_hours ?? 0) * 4) / 4, 2) }}">
                    </td>
                    <td class="border border-gray-200 px-1 py-1.5 text-center text-xs">
                        <span id="ot3-{{ $entry->id }}">
                            @if($entry->ot_ph_hours > 0)
                                {{ number_format(floor($entry->ot_ph_hours * 4) / 4, 2) }}
                            @else
                                <span class="text-gray-400">-</span>
                            @endif
                        </span>
                        <input type="hidden" id="ot3-input-{{ $entry->id }}" name="entries[{{ $entry->id }}][ot_ph_hours]" value="{{ number_format(floor(($entry->ot_ph_hours ?? 0) * 4) / 4, 2) }}">
                    </td>
                </tr>
            @endforeach
            @endforeach
        ...
        <tfoot class="bg-gray-50">
            <tr>
                <td colspan="4" class="border border-gray-200 px-2 py-1.5 text-right font-semibold">TOTAL:</td>
                <td class="border border-gray-200 px-1 py-1.5 text-center font-bold">
                ...
                <td class="border border-gray-200 px-1 py-1.5 text-center font-bold">
                    @php
                        $totalActualExec = $otForm->entries->sum(fn($e) => floor(abs($e->actual_total_hours ?? 0) * 4) / 4);
                    @endphp
                    {{ number_format($totalActualExec, 2) }}
                </td>
                <td colspan="3" class="border border-gray-200"></td>
            </tr>
        </tfoot>
